...
struct Wrapper : Learner, Terminator {
  ...
  struct {
    ...
    // PAAVO:
    void (*trail_function) (void *, unsigned long, int *);
  } learner;
  ...
  // PAAVO:
  void learn_trail (unsigned long size, int* data) {
    learner.trail_function (learner.state, size, data);
  }
...
// PAAVO:
void ccadical_set_learn_trail(CCaDiCaL * ptr, void *state, void (*trail)(void * state, unsigned long size, int * trail)) {
  Wrapper * wrapper = (Wrapper *) ptr;
  wrapper->learner.state = state;
